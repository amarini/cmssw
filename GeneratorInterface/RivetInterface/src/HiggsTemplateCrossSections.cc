...
namespace Rivet {
    ...
    class HiggsTemplateCrossSections : public Analysis {
        ...
        public:
            ...
            double getPtHjj(const Jets &jets, const Particle &higgs) {
                if (jets.size() <2) return 0;
                const FourMomentum &j1=jets[0].momentum(), &j2=jets[1].momentum();
                return (j1+j2+higgs.momentum()).pt();
            }
            ...
            HTXS::Stage1p1::Category getStage1p1Category(const HTXS::HiggsProdMode prodMode,
                    const Particle &higgs,
                    const Jets &jets,
                    const Particle &V) {
    ...
    }; // HiggsTemplateCrossSection class
